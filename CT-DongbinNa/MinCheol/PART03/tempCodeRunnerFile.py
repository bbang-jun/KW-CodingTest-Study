...
def solution(key, lock):
    ...
    # Moved_Key  = [[0]*Len_Key for _ in range(Len_Key)]
    Holes = 0
    ...
    for _ in range(4): #rotate key to clock wise
        ...
        for A in range(Len_Lock*(-1)+1,Len_Lock): #Move Up and Down
            for B in range(Len_Lock*(-1)+1, Len_Lock): #Move Right and Left
                Count=0 #Count hole that fits key

                for C in range(Len_Lock):
                    for D in range(Len_Lock):
                        if C-A<0 or Len_Lock<=C-A or D-B<0 or Len_Lock<=D-B:
                            continue 
                        if lock[C][D]==0 and New_Key[C-A][D-B]==1:
                            Count+=1
